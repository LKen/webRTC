...
export default {
  ...
  methods: {
    ...
    initCreate(videoDom) {
      ...
      // navigator.mediaDevices.getUserMedia(constraints2).then(res => { console.log(res) })
    ...
    handleMediaResolve(videoDom, stream) {
      ...
      const c = stream.getVideoTracks()[0].clone()
      const news = new MediaStream()
      c.applyConstraints(constraints2)
      news.addTrack(c)
      // const c = news.getVideoTracks()[0]
      // c.onoverconstrained = e => console.log(e)
      // c.applyConstraints(constraints2)
      // setTimeout(() => console.log(c, c.getConstraints()), 2000)

      // const ss = new MediaStream(constraints2)

      this.$refs['remoteVideo'].srcObject = news
